...
//  getBOSS(query, nRequestedHits); 
  getFlickr(query, nRequestedHits);
...
//		  .replace("[mstzb]", "b");
        ...
        newSearchResults.DisplayUrl = url;
        newSearchResults.Title = imageInfo.title;
  ...
  $.ajax({
    ...
    success: function (data, status) {
      ...
      for (i = 0; i < results.bossresponse.images.results.length; i++) {
        ...
        if (!addImgUrl(imageInfo.url)) continue;

        // Filter out known malware distributors.  YahooBOSS should get Chrome's list.
        if (InBlacklist(imageInfo.url) || InBlacklist(imageInfo.refererclickurl)) continue;	

        // Create a new item in g_searchResults.		 
        var newSearchResults = new Object;
        newSearchResults.MediaUrl = imageInfo.url;
        newSearchResults.query = query;
        newSearchResults.rank = i;
        newSearchResults.nDisplayed = 0;
        newSearchResults.priority = priority;
        newSearchResults.DisplayUrl = imageInfo.refererclickurl;
        newSearchResults.Title = imageInfo.title;
        newSearchResults.loading = false;
        newSearchResults.loaded = false;
        newSearchResults.deleted = false;
